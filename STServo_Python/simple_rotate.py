...
Position is a raw register value 0-4095 (0-360 deg). If target_position is
omitted, a random position is picked each run (so repeated runs visibly
move the servo instead of settling at the same spot).
"""
import sys
import random
import time
...
TARGET = int(sys.argv[4]) if len(sys.argv) > 4 else random.randint(0, 4095)
